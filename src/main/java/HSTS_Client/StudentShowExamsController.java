		...
		Platform.runLater(() -> {
			...
			for (int i = 0; i < exams.size(); i++) {
				...
				for (int j = 0; j < exams.get(i).getQuestions().size(); j++) {
					...
					Text gradeTextField2 = new Text(Integer.toString(pointsForQuestion) + "/" +Integer.toString(exams.get(i).getQuestionGrade().get(j)));
				...
				Text editTime2 = new Text(Integer.toString(examsOfStudent.get(i).getExecTime())+" minute(s) out of "+exams.get(i).getExamTime());
